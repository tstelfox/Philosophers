...
function formatData(data) {
	return data.split("\n").map(v => {
		...
		else if (v.includes("grabs"))
			output.type = behaviour.Taking;
		else if (v.includes("is sleeping"))
			output.type = behaviour.Sleeping;
		else if (v.includes("is eating"))
			output.type = behaviour.Eating;
		else if (v.includes("is thinking"))
			output.type = behaviour.Thinking;
		else
			output.type = behaviour.Unknown;
        if (output.type == behaviour.Unknown) {
            output.phil = -1;
		    output.time = 0;
		    output.line = v;
		    return output;
        }
       // CHANGE THESE TOO (split to space)
		output.phil = parseInt(v.split("|")[3]);
		output.time = BigInt(v.split("|")[1]);
		output.line = v;
		return output;
	})
